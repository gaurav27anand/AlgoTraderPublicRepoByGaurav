...
from Packages.Tester import MACD_Trend_checker
from Packages.Tester import RSI_Trend_checker
    ...
    def run_real_time_trader(self, api_key, api_secret):
        ...
        while True:
            ...
            if df is not None and not df.empty:
                ...
                if df['close'].iloc[-2] <= df['Lower'].iloc[-2] and self.starting_balance > self.min_balance_threshold and units > 0 and trade_amount >= self.min_trade_amount:
                    # Buy Signal
                    logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"\t\t\t ----Buy Signal Received | Current Price: {latest_price}----\n\nChecking Level 1 and 2 MACD Trends Before Buying", type="dot")
                    level1, level2 = MACD_Trend_checker.get_current_trend(self.symbol, self.bar_length)
                    if level1 == "uptrend" and level2 == "uptrend":
                        logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"MACD Approves Buying", type="dot")
                        buy_signal = True
                        
                    elif level1 == "downtrend" and level2 == "downtrend":
                        logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"MACD DENIES Buying", type="dot")
                        buy_signal = False
                        
                    elif level1 == "downtrend" and level2 == "uptrend":
                        logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"MACD DENIES Buying", type="dot")
                        buy_signal = False
                        
                    elif level1 == "uptrend" and level2 == "downtrend":
                        logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"Immediate Market is UP but long market is going down trend, going for a second check of RSI.", type="hyphen")
                        state, trend = RSI_Trend_checker.get_rsi_market_state(symbol=self.symbol, window=14, steps_back=4, bar_length=self.bar_length)
                        
                        if state == "Buy":
                            logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"RSI Approves Buying", type="dot")
                            buy_signal = True
                        elif state == "Sell":
                            logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"RSI DENIES Buying", type="dot")
                            buy_signal = False
                        elif state == "Flat" and trend == "Climbing":
                            logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"RSI Conditionally Approves Buying ### KEEP AN EYE ON PRICES ###", type="dot")
                            buy_signal = True
                        else:
                            logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"RSI DENIES Buying", type="dot")
                            buy_signal = False
                ...
                # elif df['high'].iloc[-2] > df['Upper'].iloc[-2] and df["close"].iloc[-2] < df["MA"].iloc[-2] and df["close"].iloc[-3] < df["MA"].iloc[-2] and self.position > 0 :
                    ...
                    logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"\t\t\t ----Sell Signal Received  | Current Price : {latest_price} ----\n\nChecking Level 1 and 2 MACD Trends Before Selling", type="dot")
                    level1, level2 = MACD_Trend_checker.get_current_trend(self.symbol, self.bar_length)
                    if level1 == "downtrend" and level2 == "downtrend":
                        logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"MACD Approves Selling", type="dot")
                        sell_signal = True
                        
                    elif level1 == "uptrend" and level2 == "uptrend":
                        logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"MACD DENIES Selling", type="dot")
                        sell_signal = False
                        ...
                        logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"Immediate Market is Down but long market is UP, going for a second check of RSI.", type="hyphen")
                        state, trend = RSI_Trend_checker.get_rsi_market_state(symbol=self.symbol, window=14, steps_back=4, bar_length=self.bar_length)
                        
                        if state == "Sell":
                            logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"RSI Approves Selling", type="dot")
                            sell_signal = True
                        elif state == "Buy":
                            logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"RSI DENIES Selling", type="dot")
                            sell_signal = False
                        elif state == "Flat" and trend == "Plunging":
                            logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"RSI Approves Selling", type="dot")
                            sell_signal = True
                        else:
                            logger.print_centered_box(exchange="coindcx", symbol=self.symbol, id=RealTimeBollingerBandsTrader.starting_timestamp,  content=f"RSI DENIES Selling", type="dot")
